...
function recarga()
{
	...
	$("#datos").load('ajax/getDepartamento.php', { },function(response,status,xhr){
		if(status == 'success')
		{
...
var ar=$("#dep").val().length;

limpia_campos();
	var llena = 1;
	
	if(ar==0){	
		$("#dep").addClass("alert-danger");
		llena = 0;	
	}
	if (llena==0)
	...
	if (llena == 1)//valido logica
	{
			...
			<!--ajax inicio-->
			var url ='ajax/agrega_departamento.php';
			$.ajax({
				type:'POST',
				url:url,
...
//metodo de edicion
	...
	if ($("#dep").hasClass("alert-danger"))
	$("#dep").removeClass("alert-danger");
	}